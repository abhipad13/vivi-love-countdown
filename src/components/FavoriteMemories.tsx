    ...
    <div className="animate-fade-in-up animate-delay-2">
      ...
      <div className="relative max-w-6xl mx-auto">
              ...
              <CarouselItem key={pageIndex}>
                <div className="relative p-8 bg-white/90 backdrop-blur-sm rounded-3xl shadow-2xl border-2 border-romantic-pink/30 min-h-[600px]">
                  ...
                  <div className="space-y-8">
                    {page.map((memory, memoryIndex) => (
                      <div 
                        key={memory.id}
                        className={`flex flex-col ${memoryIndex % 2 === 0 ? 'lg:flex-row' : 'lg:flex-row-reverse'} gap-6 items-center`}
                      >
        ...
        <p className="text-center mt-6 text-romantic-deep/70 font-elegant text-sm">
          ← Click the arrows to flip through our memory book →
        </p>
      </div>
    </div>
  );
